##########################################################################################################################
# File automatically-generated by tool: [projectgenerator] version: [4.10.0-B28] date: [Fri Sep 04 16:14:37 EDT 2026] 
##########################################################################################################################
...
# building variables
######################################
# debug build?  1 = debug (-Og, dwarf-2)   0 = release (-O2 -g3, default)
DEBUG ?= 0
...
# release optimization (override on the command line, e.g. make OPT=-Os)
OPT = -O2
endif
...
CFLAGS += $(MCU) $(C_DEFS) $(C_INCLUDES) $(OPT) -Wall -Wdouble-promotion -fdata-sections -ffunction-sections
...
#######################################
# flash (SWD via STM32CubeProgrammer)
#######################################
STM32_PROGRAMMER ?= STM32_Programmer_CLI

flash: $(BUILD_DIR)/$(TARGET).elf
	$(STM32_PROGRAMMER) -c port=SWD -w $< -v -rst
